...
const example1 = `px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
(async function main () {
  for (let [name, rawData] of [['example', example1], ['input', input]]) {
    const data = rawData
      .split('\n\n')
      .map(p => p.split('\n'))
    ...
    const workflows = data[0]
      .reduce((acc, line) => {
        const [_, name, steps] = [...line.matchAll(/(\w+)\{(.+)\}/g)][0]
        acc[name] = steps.split(',').map(v => {
          let [_, cond, nextName] = [...v.matchAll(/([xmas<>\d]+:)*(\w+)/g)][0]
          cond &&= { var: cond.slice(0, 1), op: cond.slice(1, 2), val: +cond.slice(2, -1) }
          return cond ? { cond, nextName } : { nextName }
        })
        return acc
      }, {})
    ...
    for (let part2 of [false, true]) {
      const parts = part2
        ...
        : data[1]
          .map(line => {
            const [_, vars] = [...line.matchAll(/\{(.+)\}/g)][0]
            return Object.fromEntries(vars.split(',').map(v => v.split('=')).map(([_, v]) => [_, +v]))
          })
      let ans = 0
      ...
      function dfs (xmas, curr)  {
        if (curr == 'R') return
        if (curr == 'A') {
          ans += part2
            ? Object.values(xmas).reduce((acc, [min,max]) => acc * (max - min + 1), 1)
            : Object.values(xmas).reduce((acc, v) => acc + v)
          return
        }

        let ranges = xmas

        for (let { cond, nextName } of workflows[curr]) {
          if (!cond) dfs(ranges, nextName)
          if (cond) {
            if (part2) {
              const rat = ranges[cond.var]

              if (cond.op == '<' && rat[0] < cond.val) {
                const range1 = [rat[0], Math.min(cond.val - 1, rat[1])]
                const range2 = [Math.min(cond.val, rat[1]), rat[1]]

                dfs({...ranges, [cond.var]: range1 }, nextName)
                ranges = {...ranges, [cond.var]: range2 }
              } else if (cond.op == '>' && rat[1] > cond.val) {
                const range1 = [Math.max(cond.val + 1, rat[0]), rat[1]]
                const range2 = [rat[0], Math.max(cond.val, rat[0])]

                dfs({...ranges, [cond.var]: range1 }, nextName)
                ranges = {...ranges, [cond.var]: range2 }
              }
            } else if (
              (cond.op == '<' && ranges[cond.var] < cond.val) ||
              (cond.op == '>' && ranges[cond.var] > cond.val)
            ) {
              dfs(ranges, nextName);
              break
