...
function perimeter (path) {
  ...
  for (let i = 1; i < path.length; i++) {
    const [y1, x1] = path[i - 1]
    const [y2, x2] = path[i]
    P += Math.abs(x2 - x1) + Math.abs(y2 - y1)
  }
...
(async function main () {
  for (let [name, data] of [['example', example1], ['input', input]]) {
    const line = data
      .split('\n')
      .map(line => {
        const [d, v, hex] = line.split(' ')
        const dist = parseInt(hex.slice(2, -2), 16)
        const dir = hex.slice(-2, -1)
        return [{'R':0,'D':1,'L':2,'U':3}[d], +v, +dir, +dist]
      })
      const l = line.length

    for (let part2 of [false, true]) {
      const plan = line.map(p => part2 ? p.slice(2) : p.slice(0, 2))
      const path = [[0, 0]]
