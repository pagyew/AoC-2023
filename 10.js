...
(async function main () {
  ...
  // console.log(sketch.join('\n'));
  // console.log('\n');
  const n = sketch.length
  const m = sketch[0].length
  const startIndex = input.replaceAll(/\n/g, '').indexOf('S');
  const startPoint = [startIndex % m, startIndex / m >> 0]
  const map = Array.from({ length: n }, _ => Array(m).fill('-'))

  // console.log(startPoint, `${n}x${m}`);

  map[startPoint[1]][startPoint[0]] = 'o';

  const availableDirs = Object.keys(dirs).filter(dir => {
    const [dx, dy] = dirs[dir]
    const [x, y] = startPoint
    const point = sketch[y + dy]?.[x + dx]

    return point in tiles && tiles[point].from[oppositeDirs[dir]]
  })

  const startTile = Object.entries(tiles)
    .find(([_,{ from }]) => availableDirs[0] in from && availableDirs[1] in from)[0]

  sketch[startPoint[1]] = sketch[startPoint[1]].replace('S', startTile)
  ...
  const l = {
    x: startPoint[0],
    y: startPoint[1],
    from: oppositeDirs[availableDirs[0]],
    to: availableDirs[0]
  }
  const r = {
    x: startPoint[0],
    y: startPoint[1],
    from: oppositeDirs[availableDirs[1]],
    to: availableDirs[1]
  }

  while (l.x != r.x || l.y != r.y || sketch[l.y][l.x] == sketch[startPoint[1]][startPoint[0]]) {
    const lD = dirs[l.to];
    const rD = dirs[r.to];
    const lFrom = sketch[l.y][l.x]
    const rFrom = sketch[r.y][r.x]
    ...
    const lTo = sketch[l.y][l.x];
    const rTo = sketch[r.y][r.x];

    // console.log(`l from ${l.from} (${lFrom}) to ${l.to} (${lTo})`);
    // console.log(`r from ${r.from} (${rFrom}) to ${r.to} (${rTo})`);
    // console.log(`count: ${count}`);

    l.to = tiles[lTo].from[l.from].to
    l.from = oppositeDirs[l.to]
    r.to = tiles[rTo].from[r.from].to
    r.from = oppositeDirs[r.to]
  ...
  for (let y = 0; y < n; y++) {
    ...
    for (let x = 0; x < m; x++) {
      if (map[y][x] == 'o') {
        const tile = sketch[y][x]

        if (tile == '|') {
          inside = !inside
        } else if (openedTile) {
          if (tile in tiles[openedTile].close) {
            let crosses = tiles[openedTile].close[tile]
            inside = crosses == 2 ? inside : !inside
            openedTile = ''
          }
        } else {
          openedTile = tile
  ...
  // console.log(map.map((row, i) => i + ' ' + row.join('')).join('\n'));
  console.log(count);
